...
def most_reviewd_products():
    # Group by product ID and count the number of reviews for each product
    product_reviews_count = create_df_cleaned_data()

    product_reviews_count_groupby = product_reviews_count.groupby('asin').size().reset_index(name='review_count')
    
    # # Sort the products by review count in descending order and select the top 10
    top_10_products = product_reviews_count_groupby.nlargest(10, 'review_count')
    
    # # Merge with the original dataset to get the average rating for each product
    top_10_products_with_ratings = pd.merge(top_10_products, create_df_cleaned_data().groupby('asin')['overall'].mean().reset_index(name='average_rating'), on='asin')
    
    # Display the top 10 most reviewed products with their average ratings
    # display(top_10_products_with_ratings)
    return top_10_products_with_ratings



create_df_cleaned_data()
